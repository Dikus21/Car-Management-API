...
export default class UserController {
    ...
    public async loginUser(req: Request, res: Response) {
        try {
            ...
            const refreshToken = jwt.sign({userId, userName, userEmail, userRole}, process.env.REFRESH_TOKEN_SECRET!, {
                expiresIn: '1d',
            });
    ...
    public async getAllUsers(req: Request, res: Response) {
        try {
            const users = await User.find({select: ['id', 'name', 'email', 'role','refreshToken']});
